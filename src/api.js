const GRAPHQL_ENDPOINT = 'https://indexer.hyperindex.xyz/8fd74dc/v1/graphql'
...
const USD_CENTS_DECIMALS = 2

// Payment method hash to name mapping
...
const MAKER_DETAIL_QUERY = `
  query MakerDetail(
    ...
    currencyStats: MakerCurrencyStats(
      where: { id: { _like: $makerPrefix } }
      order_by: { totalAmountTaken: desc }
      limit: 1
    ) {
...
function convertUnits(value, decimals = 0) {
  if (!value) return 0
  return Number(value) / 10 ** decimals
}

function getPaymentMethodName(hash) {
  if (!hash) return 'N/A'
  return PAYMENT_METHODS[hash.toLowerCase()] || 'UNKNOWN'
}

function getCurrencyCode(hashOrCode) {
  if (!hashOrCode) return 'USD'
  // If it's already a short code (3 letters), return it
  if (hashOrCode.length <= 4) return hashOrCode.toUpperCase()
  // Otherwise look up the hash
  return CURRENCIES[hashOrCode.toLowerCase()] || 'USD'
}

export async function fetchMakerStats(address) {
  // Normalize address
  const normalizedAddress = address.toLowerCase()
  
  // Construct makerId: chainId_address
  const makerId = `${CHAIN_ID}_${normalizedAddress}`
  const makerPrefix = `${makerId}_%`
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch maker stats')
  }
  ...
  const topCurrency = data.currencyStats?.[0]

  return {
    volume: convertUnits(maker.totalAmountTaken, USDC_DECIMALS),
    profit: convertUnits(maker.realizedProfitUsdCents, USD_CENTS_DECIMALS),
    deposits: maker.fulfilledIntents || 0,
    currency: getCurrencyCode(topCurrency?.currencyCode),
    platform: getPaymentMethodName(topPlatform?.paymentMethodHash),
  }
}